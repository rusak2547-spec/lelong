  ...
  return (
    <div className="group relative bg-white dark:bg-zinc-900/50 rounded-3xl border border-zinc-100 dark:border-zinc-800 overflow-hidden hover:shadow-2xl hover:shadow-zinc-200/50 dark:hover:shadow-zinc-950/50 transition-all duration-500 flex flex-col h-full">
      <div className="relative aspect-[4/5] w-full overflow-hidden bg-zinc-100 dark:bg-zinc-800">
        <Image src={product.imageUrl} alt={product.title} fill sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" className="object-cover transition-transform duration-700 group-hover:scale-110" />
        <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
        <div className="absolute top-3 left-3 flex flex-col gap-2">
           <Badge className={`backdrop-blur-md border-0 px-3 py-1 font-semibold tracking-wide ${product.status === 'AVAILABLE' ? 'bg-emerald-500/90 text-white shadow-lg shadow-emerald-500/20' : 'bg-rose-500/90 text-white shadow-lg shadow-rose-500/20'}`}>
        ...
        <div className="absolute top-3 right-3">
           <Badge variant="secondary" className="backdrop-blur-md bg-white/90 dark:bg-black/80 font-medium border border-white/20">
              {product.category === 'GOLD' ? 'EMAS' : product.category || 'EMAS'}
           </Badge>
        </div>
      </div>
      ...
      <div className="flex flex-col flex-1 p-5 md:p-6">
        <div className="mb-4 flex-1">
           <div className="flex items-center gap-2 text-xs text-zinc-500 dark:text-zinc-400 mb-2">
              ...
              <span>{product.createdAt ? new Date(product.createdAt).toLocaleDateString('id-ID') : '-'}</span>
           </div>
        ...
        <div className="grid grid-cols-2 gap-3 mb-6 bg-zinc-50 dark:bg-zinc-800/50 p-3 rounded-2xl border border-zinc-100 dark:border-zinc-800">
           <div className={`flex flex-col ${!isGold ? 'col-span-2' : ''}`}>
              <span className="text-[10px] uppercase tracking-wider text-zinc-500 font-semibold mb-1 flex items-center gap-1">
                <Tag className="w-3 h-3" /> Harga Buka
              </span>
              <span className="font-bold text-lg text-blue-600 dark:text-blue-400 truncate">
                 <span className="text-xs text-zinc-400 font-normal mr-1">Rp</span>
        ...
        <Button className="w-full rounded-xl h-12 font-semibold text-base shadow-sm group-hover:shadow-blue-500/25 transition-all duration-300 bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 hover:bg-black dark:hover:bg-zinc-200" render={<Link href={`/product/${product.id}`} />}>
          Lihat Detail
